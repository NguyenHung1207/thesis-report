...
\begin{itemize}
    ...
    \item \textbf{Kết hợp agent và replay script}: agent hỗ trợ tạo script ban đầu; hệ thống cho phép tester chạy lại replay script đối với các kịch bản đã ổn định trong kiểm thử hồi quy.
...
Replay script được sử dụng khi test case đã có kịch bản được xác nhận. Chế độ này thực thi theo các step đã lưu nên có tính xác định hơn về luồng thao tác, phù hợp với các kịch bản ổn định trong kiểm thử hồi quy.
...
Test suite gom nhiều test case đã được lưu trong cây thư mục thành một nhóm chạy. Đối với các test case đã có replay script, hệ thống hỗ trợ cấu hình thực thi bằng replay script để phục vụ kiểm thử hồi quy.
...
Agent linh hoạt; replay script có tính xác định hơn về luồng thao tác và phù hợp với các kịch bản hồi quy đã ổn định. &
...
Nhìn chung, thiết kế hướng đến sự cân bằng giữa khả năng hỗ trợ tạo kịch bản của AI Agent và tính xác định, khả năng tái sử dụng của replay script.
...
Trong các lần kiểm thử hồi quy tiếp theo, tester có thể thực thi replay script bằng Playwright thay vì yêu cầu LLM suy luận lại ở từng bước. Kiến trúc này tạo điều kiện giảm số lần gọi mô hình trong các lần chạy hồi quy và nâng cao khả năng tái sử dụng test script.
