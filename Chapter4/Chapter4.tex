...
Giao diện được thiết kế theo vòng đời kiểm thử: xác thực tester, quản lý project, sinh và lựa chọn test case, tạo automation test script, chuẩn bị dữ liệu kiểm thử, tổ chức test case và xem kết quả thực thi.

Các màn hình chính gồm đăng nhập, đăng ký, Dashboard, Generate Testcase, Test Case, Test Case Detail, Test Suite Detail, Data, Objects và Test Runs.

Trang \textit{Generate Testcase} là khu vực để tester nhập yêu cầu bằng ngôn ngữ tự nhiên, sinh danh sách test case đề xuất và lựa chọn các test case phù hợp. Giao diện có thể tổ chức thành nhiều section làm việc để tester xử lý các nhóm yêu cầu khác nhau. Đây là cách tổ chức giao diện, không được xem là một chức năng nghiệp vụ độc lập.

Các test case được lựa chọn được sử dụng để chạy bằng agent và tạo automation test script. Sau khi script được tạo và xác nhận, test case được chuyển sang trang \textit{Test Case} và tổ chức theo cấu trúc cây thư mục.
...
Lưu từng lần sinh và các test case đề xuất tại trang
\textit{Generate Testcase}. Candidate chỉ được chuyển thành test case
chính thức sau khi được tester lựa chọn và hoàn tất quá trình tạo script. \\
\hline
\end{tabular}
\end{table}
...
Test case do AI sinh ban đầu được lưu dưới dạng candidate. Chỉ những
candidate được tester lựa chọn và hoàn tất quá trình tạo script mới
được đưa vào kho test case chính thức. \\
\hline
...
Các test case được sinh được hiển thị trong khu vực làm việc để tester xem xét, chỉnh sửa và lựa chọn. Những test case chưa được lựa chọn chỉ là kết quả đề xuất và chưa được đưa vào kho test case chính thức của project.

Tester có thể sử dụng các test case đã lựa chọn làm đầu vào cho quá trình chạy bằng agent nhằm tạo automation test script. Sau khi quá trình tạo script hoàn thành và được xác nhận, test case cùng script tương ứng được chuyển sang trang \textit{Test Case} để lưu trữ và quản lý.
...
Agent được sử dụng trong giai đoạn thực thi ban đầu để khám phágiao diện và tạo automation test script từ test case đã được tester lựa chọn.
...
Test suite gom nhiều test case đã được lưu trong cây thư mục thành một nhóm chạy. Đối với các test case đã có replay script, hệ thống ưu tiên thực thi bằng replay script để phục vụ kiểm thử hồi quy. Agent được sử dụng khi cần tạo mới hoặc cập nhật script do test case chưa có kịch bản phù hợp hoặc luồng nghiệp vụ đã thay đổi đáng kể.
...
Trước khi chạy, hệ thống kiểm tra cấu trúc dataset với các biến của replay script dựa trên tên trường và kiểu dữ liệu. Cấu hình không đầy đủ hoặc dataset không tương thích sẽ bị chặn.
