...
Hệ thống là nền tảng web hỗ trợ tester xây dựng, quản lý và thực thi kiểm thử tự động đối với các ứng dụng website. Tester có thể tạo và quản lý project, sinh test case từ mô tả bằng ngôn ngữ tự nhiên, lựa chọn các test case phù hợp, quản lý dữ liệu kiểm thử, chạy test bằng agent hoặc replay script và xem kết quả thực thi.

Quá trình sinh test case được thực hiện tại trang \textit{Generate Testcase}. Tester nhập yêu cầu hoặc mục tiêu kiểm thử bằng ngôn ngữ tự nhiên, sau đó hệ thống sử dụng mô hình ngôn ngữ lớn để sinh danh sách test case đề xuất. Giao diện có thể hỗ trợ nhiều khu vực làm việc để tester xử lý các nhóm yêu cầu khác nhau, nhưng đây là cách tổ chức thao tác trên giao diện, không phải một chức năng nghiệp vụ độc lập.

Sau khi test case được sinh, tester xem xét, chỉnh sửa và lựa chọn các test case phù hợp. Những test case được lựa chọn có thể được sử dụng để chạy bằng agent và tạo automation test script. Khi script được tạo thành công và được tester xác nhận, test case cùng script tương ứng được lưu tại trang \textit{Test Case} theo cấu trúc cây thư mục để phục vụ quản lý và tái sử dụng.
...
Bộ xử lý nghiệp vụ &
Kiểm tra dữ liệu, quản lý nghiệp vụ và điều phối các yêu cầu sinh test case, sinh test script và thực thi kiểm thử. \\
\hline

Kho dữ liệu &
Lưu trữ tester, project, test case, cấu trúc cây thư mục, dataset, test run, replay script, evidence và Object Repository. \\
\hline

Thành phần thực thi kiểm thử &
Chạy test bằng agent hoặc replay script, thao tác trên trình duyệt, tạo automation test script và ghi nhận kết quả. \\
\hline
...
\begin{itemize}
    ...
    \item Xem xét, chỉnh sửa và lựa chọn các test case phù hợp.
    \item Sinh automation test script từ các test case đã lựa chọn.
    \item Quản lý test case và test script theo cấu trúc cây thư mục.
...
Cho phép agent tiếp nhận test case đã được lựa chọn, quan sát giao diện, suy luận và thực hiện các thao tác kiểm thử trên website mục tiêu. &
Must \\
\hline

FR09 &
Sinh và quản lý replay script &
Cho phép hệ thống tạo replay script từ lần chạy agent thành công; cho phép tester xem, chỉnh sửa và xác nhận trước khi lưu test case cùng script vào cây thư mục tại trang \textit{Test Case}. &
Must \\
\hline

FR10 &
Chạy replay script &
Cho phép tester chạy lại kịch bản đã lưu mà không cần LLM suy luận ở từng bước. &
Must \\
\hline
...
FR13 &
Locator fallback &
Cho phép agent hoặc thành phần thực thi thử phương thức định vị khác khi locator ban đầu không còn phù hợp. &
Should \\
\hline
...
\caption{Use case sinh, lựa chọn test case và chạy bằng agent}
\label{fig:usecase_agent_testing}
\end{figure}

Hình~\ref{fig:usecase_agent_testing} mô tả quá trình tester nhập yêu cầu tại trang \textit{Generate Testcase}, sinh danh sách test case, xem xét, chỉnh sửa, lựa chọn test case và chạy các test case đã chọn bằng agent. Giao diện có thể chia thành nhiều khu vực làm việc để tester xử lý các nhóm yêu cầu khác nhau, nhưng không được tách thành một use case độc lập.

Khi lần chạy agent thành công, hệ thống tạo replay script để tester xem xét và xác nhận. Chức năng quản lý dữ liệu kiểm thử có thể được thực hiện độc lập tại module Data.
...
Hình~\ref{fig:usecase_replay_data_suite} mô tả các chức năng lưu và quản lý test case theo cấu trúc cây thư mục, quản lý replay script, chạy replay script với một hoặc nhiều dòng dữ liệu và quản lý test suite.

Sau khi replay script được tester xác nhận, test case cùng script được lưu tại trang \textit{Test Case}. Tester có thể phân nhóm các test case theo thư mục, module hoặc luồng nghiệp vụ để thuận tiện quản lý và tái sử dụng. Tester có thể quản lý test suite mà không bắt buộc phải chạy replay script.
...
UC01 &
Đăng ký tài khoản &
Cho phép tester tạo tài khoản mới. &
...
Thực thi test case đã lựa chọn bằng agent và browser automation. &
...
Xem xét, chỉnh sửa và xác nhận replay script được tạo từ lần chạy agent thành công. &
Test case và replay script được lưu vào cây thư mục tại trang \textit{Test Case}. \\
\hline
...
Chương này đã trình bày tổng quan hệ thống, đối tượng sử dụng, yêu cầu chức năng, yêu cầu phi chức năng và các use case chính. Quy trình chính bao gồm sinh và lựa chọn test case, chạy bằng agent để tạo replay script, xác nhận và lưu test case theo cấu trúc cây thư mục, sau đó tái sử dụng replay script trong các lần kiểm thử tiếp theo.
